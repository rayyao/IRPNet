    ...
    def testing (self, epoch):
        ...
        with torch.no_grad():
            for i, ( data, labels) in enumerate(tbar):
                ...
                PD, FA = self.PD_FA.get(len(self.test_data))
                _, mean_IOU = self.mIoU.get()
                _, normal_IOU = self.nIoU.get()
                ture_positive_rate, false_positive_rate, recall, precision = self.ROC.get()

                tbar.set_description('Epoch %d, test loss %.4f, IoU: %.4f, nIoU: %.4f' % (epoch, losses.avg, mean_IOU, normal_IOU))
            test_loss=losses.avg

        # save high-performance model
        save_model(mean_IOU, self.best_iou, self.save_dir, self.save_prefix, self.train_loss, test_loss, epoch, self.model.state_dict())

        if mean_IOU > self.best_iou:
            self.best_iou = mean_IOU
        if normal_IOU > self.best_niou:
            self.best_niou = normal_IOU
        if FA[0]*1000000 > 0 and FA[0]*1000000 < self.best_fa:
            self.best_fa = FA[0]*1000000
        if PD[0] <=1 and PD[0] > self.best_pd:
            self.best_pd = PD[0]

        print(f"IoU: {mean_IOU}, nIoU: {normal_IOU}, FA: {FA[0]*1000000}, PD: {PD[0]}")
        print(f"best_IoU: {self.best_iou}, best_nIoU: {self.best_niou}, best_FA: {self.best_fa}, best_PD: {self.best_pd}")
