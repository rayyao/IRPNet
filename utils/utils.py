...
def save_model(current_value, best_value, save_dir, save_prefix, train_loss, test_loss, epoch, net,
               recall=None, precision=None, save_mIoU_dir=None, save_other_metric_dir=None,
               metric_type='mIoU'):
    if metric_type == 'Fa':
        if current_value < best_value:
            ...
            if recall is not None and precision is not None and save_mIoU_dir is not None and save_other_metric_dir is not None:
                save_model_and_result(dt_string, epoch, train_loss, test_loss, best_value,
            ...
            save_ckpt(
                {
                    ...
                    'state_dict': net,
                    'loss': test_loss,
                    f'{metric_type}': current_value,
                },
                save_path=f'result/{save_dir}',
                filename=f'{metric_type}_{save_prefix}.pth.tar'
            )
...
def save_result_for_test(dataset_dir, st_model, epochs, best_iou, best_niou, pd, fa, best_f1):
    with open(dataset_dir + '/' + 'value_result'+'/' + st_model +'_best_IoU.log', 'a') as f:
        now = datetime.now()
        dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
        f.write('{} - epoch: {:04d}\n'.format(dt_string, epochs))
        f.write('IoU-----:')
        f.write('   ')
        ...
        f.write('nIoU----:')
        f.write('   ')
        f.write(str(round(best_niou, 4)))
        f.write('\n')

        f.write('PD------:')
        f.write('   ')
        f.write(str(round(pd, 4)))
        f.write('\n')

        f.write('FA------:')
        f.write('   ')
        f.write(str(round(fa, 5)))
        f.write('\n')

        f.write('F1------:')
        f.write('   ')
        f.write(str(round(best_f1, 4)))
...
def save_Pred_GT(pred, labels, target_image_path, val_img_ids, num, suffix):
    pred_gray = pred.cpu().detach().numpy() 
    pred_gray = (pred_gray - pred_gray.min()) / (pred_gray.max() - pred_gray.min() + 1e-8) 
    pred_gray = (pred_gray * 255).astype(np.uint8)  
    
    
    img_gray = Image.fromarray(pred_gray.reshape(256, 256))
    img_gray.save(os.path.join(target_image_path, f"{val_img_ids[num]}_Gray{suffix}"))
